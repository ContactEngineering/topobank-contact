...
def contact_mechanics(
    ...
    if pressures and (
        (len(pressures) < 1) or (len(pressures) > kwargs_limits["pressures"]["maxlen"])
    ):
    ...
    for i in range(nsteps):
        ...
        make_dzi(
            ...
            f"{storage_path}/dzi/contacting-points",
            physical_sizes=topography.physical_sizes,
            unit=topography.unit,
            cmap="magma",
        )
        ...
        make_dzi(
            displacement_xy.data * get_unit_conversion_factor(topography.unit, unit),
